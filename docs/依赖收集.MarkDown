...
function defineReactive (obj, key, val, cb) {
    ...
    Object.defineProperty(obj, key, {
        ...
        get: ()=>{
            if (Dep.target) {
                /* Watcher对象存在全局的Dep.target中， 这个地方每被get一次，就add一次，不会有问题吗 */
...
总结：所谓的依赖收集是指排除render函数（同时也是模板中）没有用到的data数据。使得这些data数据改变时不会重新运行render函数进行渲染。但是以上的简易实现，会导致，一个已经被收集的data数据变化，会运行多次render函数，因为Dep.notify这个方法，是遍历dep内部的依赖的。